...
public class AcronymActivity extends Activity {
	...
	/**
	 * The implementation of the AcronymResults AIDL Interface. Should be passed
	 * to the WebService using the AcronymRequest.expandAcronym() method.
	 * 
	...
	private AcronymResults.Stub mAcronymResults = new AcronymResults.Stub() {
		/**
		 * This method is called back by the Service to return the results.
		 */
		@Override
	 ...
	 */
	private GenericServiceConnection<AcronymRequest> mServiceConnectionAsync = AcronymServiceConnectionFactory
			.newAcronumRequestConnection();
	/**
	 * This GenericServiceConnection is used to receive results after binding to
	 * the AcronymServiceSync Service using bindService().
	 */
	private GenericServiceConnection<AcronymCall> mServiceConnectionSync = AcronymServiceConnectionFactory
			.newAcronumCallConnection();
	...
	public void expandAcronymAsync(View v) {
		AcronymRequest acronymRequest = mServiceConnectionAsync.getInterface();
		...
		if (acronymRequest != null) {
			...
			try {
				...
				acronymRequest.expandAcronym(mAcronymResults, acronym);
			...
			new Thread(new Runnable() {
				public void run() {
					try {
						...
						final List<AcronymData> acronymDataList = acronymCall
	...
	public void onStart() {
		...
		if (mServiceConnectionSync.getInterface() == null)
			bindService(AcronymServiceSync.makeIntent(this),
					mServiceConnectionSync, BIND_AUTO_CREATE);
	}
